...
class DeveloperRepository extends Repository
{
    ...
    public function searchDeveloper($request)
    {
        ...
        if ($programming_language && $language) {
            return $this->model->with(['programmingLanguage', 'language'])->whereHas('language', function ($q) use ($language) {
                $q->where('code', $language);
            })->whereHas('programmingLanguage', function ($q) use ($programming_language) {
                $q->where('name', $programming_language);
            })->get();
        } elseif ($programming_language) {
            return $this->model->with(['programmingLanguage', 'language'])->whereHas('programmingLanguage', function ($q) use ($programming_language) {
                $q->where('name', $programming_language);
            })->get();
        } else {
            return $this->model->with(['programmingLanguage', 'language'])->whereHas('language', function ($q) use ($language) {
